...
def run_prediction(df: pd.DataFrame, bundle: Dict) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Neural specialists. They consume matching main meta outputs.
    for key, mask, label, seg in [
        ("site802_allocate_specialist", site802_m & alloc_m, "site802_allocate_nn", "allocate"),
        ("site802_review_specialist", site802_m & review_m, "site802_review_nn", "review"),
        ("ak_allocate_specialist", ak_alloc_m, "ak_allocate_nn", "allocate"),
    ]:
        model = models.get(key)
        if model is None or not mask.any() or seg not in seg_cache:
            continue
        seg_mask, seg_pred = seg_cache[seg]
        seg_indices = np.where(seg_mask)[0]
        local_map = pd.Series(np.arange(len(seg_indices)), index=seg_indices)
        local = local_map.loc[np.where(mask)[0]].to_numpy()
        prob = seg_pred["prob"][local]
        rank = seg_pred["rank"][local]
        aux = seg_pred["aux"][local]
        meta_x = make_meta_features(X[mask], prob, rank, aux)
        flms = np.maximum(model.predict(meta_x).reshape(-1), 0.0)
        idx = np.where(mask)[0]
        raw_units[idx] = flms * flm[idx]
        out.loc[mask, "model_path"] = label
        out.loc[mask, "pred_flms_raw"] = flms
...
with st.sidebar:
    ...
    if missing_core:
        st.warning("Prediction is disabled until required model files are added.")
    else:
        st.success("Prediction models loaded.")
    st.divider()
    st.caption("Version")
...
with files_tab:
    ...
    st.markdown("#### Notes")
    st.write("This package intentionally does not include the trained model `.npz` files if they were not supplied. Drop the model files into this folder and restart Streamlit to enable prediction.")
